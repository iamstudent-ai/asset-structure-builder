// AssetDetail.tsx — Detail view with warranty badges.
...
const Field = ({
  ...
  label: string; value: string | number; editing: boolean; editable: boolean;
...
const AssetDetail = ({ asset, onBack, onSave, readOnly = false }: AssetDetailProps) => {
  ...
  const validate = (): boolean => {
    ...
    for (const field of REQUIRED_FIELDS) {
      if (isEditable(field) && !String(draft[field]).trim()) newErrors[field] = "This field is required";
    }
    setErrors(newErrors);
